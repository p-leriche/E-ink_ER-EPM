...
void Paint::DrawShadedPixel(int x, int y) {
	DrawPixel(x, y, ((x ^ y) & 1));
...
/**
*  @brief: this draws a shaded rectangle
*/
void Paint::DrawShadedRectangle(int x0, int y0, int x1, int y1) {
    int min_x, min_y, max_x, max_y;
    min_x = x1 > x0 ? x0 : x1;
    max_x = x1 > x0 ? x1 : x0;
    min_y = y1 > y0 ? y0 : y1;
...
/**
*  @brief: this draws a speckled rectangle
*/
void Paint::DrawSpeckledRectangle(int x0, int y0, int x1, int y1, int density) {
    int min_x, min_y, max_x, max_y;
    min_x = x1 > x0 ? x0 : x1;
    max_x = x1 > x0 ? x1 : x0;
    min_y = y1 > y0 ? y0 : y1;
...
/**
*  @brief: this draws a shaded circle
*/
void Paint::DrawShadedCircle(int x, int y, int radius) {
    /* Bresenham algorithm */
    int x_pos = -radius;
    int y_pos = 0;
    int err = 2 - 2 * radius;
    ...
    do {
        DrawShadedPixel(x - x_pos, y + y_pos);
        DrawShadedPixel(x + x_pos, y + y_pos);
        DrawShadedPixel(x + x_pos, y - y_pos);
        DrawShadedPixel(x - x_pos, y - y_pos);
//      DrawHorizontalLine(x + x_pos, y + y_pos, 2 * (-x_pos) + 1, colored);
//		DrawHorizontalLine(x + x_pos, y - y_pos, 2 * (-x_pos) + 1, colored);
		for (i = x + x_pos; i <= x - x_pos; i++) {
			DrawShadedPixel(i, y + y_pos);
		    DrawShadedPixel(i, y - y_pos);
        }
...
*  @brief: this draws a shaded circle
*/
void Paint::DrawSpeckledCircle(int x, int y, int radius, int density) {
    /* Bresenham algorithm */
    int x_pos = -radius;
    int y_pos = 0;
    int err = 2 - 2 * radius;
    ...
    do {
        DrawSpeckledPixel(x - x_pos, y + y_pos, density);
        DrawSpeckledPixel(x + x_pos, y + y_pos, density);
        DrawSpeckledPixel(x + x_pos, y - y_pos, density);
        DrawSpeckledPixel(x - x_pos, y - y_pos, density);
//      DrawHorizontalLine(x + x_pos, y + y_pos, 2 * (-x_pos) + 1, colored);
//		DrawHorizontalLine(x + x_pos, y - y_pos, 2 * (-x_pos) + 1, colored);
		for (i = x + x_pos; i <= x - x_pos; i++) {
			DrawSpeckledPixel(i, y + y_pos, density);
		    DrawSpeckledPixel(i, y - y_pos, density);
        }
